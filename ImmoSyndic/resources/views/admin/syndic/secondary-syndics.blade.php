...
<div x-data="secondarySyndicsComponent({ 
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">Gestion des Syndics Secondaires</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">Gérez les syndics secondaires qui vous assistent dans la gestion de vos immeubles.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200/60 dark:divide-slate-800/60">
                <thead class="bg-gray-50/50 dark:bg-[#090D16]/40">
                    <tr>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">Syndic Secondaire</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">Coordonnées</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">Immeuble(s) assigné(s)</th>
                    ...
                    <template x-for="syndic in paginatedItems" :key="syndic.id">
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-900/30 transition-colors duration-150">
                                ...
                                <div class="flex flex-col gap-2 max-w-[280px]">
                                    <template x-for="imm in syndic.immeubles" :key="imm.id">
                                        <div class="flex items-center justify-between gap-x-2 py-1 px-2 rounded-lg bg-gray-50 dark:bg-slate-900/30 border border-gray-200/40 dark:border-slate-800/40">
                                            ...
                                            <form :action="`/syndic/secondary-syndics/${syndic.id}/transfer-primary`" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir transférer la gestion principale de cet immeuble à ce syndic secondaire ? Vous deviendrez alors un syndic secondaire.');" class="inline-block">
                                                @csrf
                                                <input type="hidden" name="immeuble_id" :value="imm.id">
                                                <button type="submit" class="py-1 px-2 inline-flex items-center gap-x-1 text-[10px] font-bold rounded-lg border border-transparent bg-amber-500/10 text-amber-600 hover:bg-amber-500 hover:text-white transition-all duration-200" title="Transférer le rôle principal">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                <form :action="`/syndic/secondary-syndics/${syndic.id}`" method="POST" onsubmit="return confirm('Retirer l\'accès de ce syndic secondaire ?');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="py-2 px-3 inline-flex items-center gap-x-1.5 text-xs font-semibold rounded-xl border border-transparent bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white transition-all duration-200" title="Retirer l'accès">
                    ...
                    <tr x-show="filteredItems.length === 0">
                        <td colspan="4" class="px-6 py-8 text-center text-sm text-slate-450 dark:text-neutral-500">
                            <div class="flex flex-col items-center justify-center py-4">
                                ...
                                <span>Aucun syndic secondaire trouvé.</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
        ...
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
            <div class="w-full flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F]">
                <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
                    <h3 class="font-bold text-gray-800 dark:text-white text-lg">Nouveau Syndic Secondaire</h3>
                ...
                <div class="p-6">
                    <form action="{{ route('syndic.secondary-syndics.store') }}" method="POST">
                        ...
                        <div class="grid gap-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">Immeuble à assigner</label>
                        ...
                        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 pt-4">
                            <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-modal-add-secondary-syndic">Annuler</button>
                            <button type="submit" class="py-2 px-4 text-sm font-semibold bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 text-white rounded-xl shadow-md shadow-primary-500/10 transition-all duration-300">Ajouter le Syndic</button>
                        </div>
                    </form>
                </div>
            </div>
