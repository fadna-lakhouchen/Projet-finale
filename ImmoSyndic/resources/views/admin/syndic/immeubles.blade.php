...
<div x-data="syndicImmeubles({ syndicNames: { @foreach($syndics as $s) '{{ $s->id }}': '{{ addslashes($s->prenom) }} {{ addslashes($s->nom) }}', @endforeach } })">
    ...
    <!-- Modal (sans overflow-hidden sur la carte pour éviter le clipping des dropdowns) -->
                                ...
                                <div class="relative" x-data="{ openFormSyndic: false }">
                                    ...
                                    <div x-show="openFormSyndic" x-cloak
                                        ...
                                        <div @click="immeubleEnCours.syndic_id = ''; openFormSyndic = false" 
                                            class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">
                                            Aucun (Non assigné)
                                        </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div x-show="!isAddingCustomVille" class="relative" x-data="{ openFormVille: false }">
                                        ...
                                        <!-- Dropdown Panel (même style glassmorphic que les filtres) -->
                                        <div x-show="openFormVille" x-cloak
                                            ...
                                            <!-- Les 12 villes standard + villes DB fusionnées -->
                                            @foreach($allVilles as $v)
                                                <div @click="immeubleEnCours.ville = '{{ $v }}'; openFormVille = false"
                                                    ...
                                                    class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">
                                                    {{ $v }}
                                                </div>
                                            @endforeach
                                            
                                            <!-- Séparateur + Option "Autre ville..." -->
                                    ...
                                    <!-- Vue saisie manuelle (si "Autre ville...") -->
