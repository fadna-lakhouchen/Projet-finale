...
<body class="bg-gray-100 p-8 font-sans text-gray-800">
    ...
    <div class="max-w-3xl mx-auto bg-white p-10 rounded-lg shadow-md border border-gray-200">
        ...
        <div class="flex justify-between items-start border-b border-gray-200 pb-6 mb-6">
            <div>
                <h1 class="text-3xl font-bold text-blue-600">ImmoSyndic</h1>
                <p class="text-gray-500 mt-1">Gestion transparente de votre copropriété</p>
                
                <div class="mt-4">
                    <p class="font-semibold text-gray-700">Syndic responsable :</p>
                    <p>{{ Auth::user()->nom }} {{ Auth::user()->prenom }}</p>
                    <p class="text-sm text-gray-500">{{ Auth::user()->email }}</p>
                </div>
            </div>
            <div class="text-right">
                <h2 class="text-2xl font-bold text-gray-700 uppercase tracking-wider">Reçu</h2>
        ...
        <div class="flex justify-between mb-8">
            <div>
                <h3 class="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-2">Détails du Résident</h3>
            ...
            <div class="text-right">
                <h3 class="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-2">Détails de l'Immeuble</h3>
        ...
        <!-- Détails du paiement -->
        <div class="mb-8">
            <h3 class="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-4">Détails de la transaction</h3>
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-y border-gray-200">
                        <th class="py-3 px-4 font-semibold text-gray-700">Description</th>
                        <th class="py-3 px-4 font-semibold text-gray-700">Mois/Année</th>
                        <th class="py-3 px-4 font-semibold text-gray-700 text-right">Montant</th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="border-b border-gray-100">
                        <td class="py-4 px-4 text-gray-800">
                            Paiement de charge commune<br>
                            <span class="text-sm text-gray-500">Méthode : {{ ucfirst($paiement->methode_paiement) }}</span>
                            @if($paiement->reference_transaction)
                                <br><span class="text-sm text-gray-500">Réf: {{ $paiement->reference_transaction }}</span>
                            @endif
                        </td>
                        <td class="py-4 px-4 text-gray-800">
        ...
        <div class="flex justify-end mb-8">
            <div class="w-1/2 bg-blue-50 p-4 rounded-lg">
                <div class="flex justify-between items-center">
                    <span class="font-bold text-gray-700">Total Payé :</span>
                    <span class="text-2xl font-bold text-blue-700">{{ number_format($paiement->montant, 2, ',', ' ') }} DH</span>
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-gray-200 pt-6 mt-8 text-center text-sm text-gray-500">
            <p>Ce document sert de reçu officiel pour le paiement des charges de copropriété.</p>
