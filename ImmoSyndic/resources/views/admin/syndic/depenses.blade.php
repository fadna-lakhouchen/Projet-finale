...
<div x-data="syndicDepenses" class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">Enregistrez et justifiez chaque centime dépensé pour vos copropriétés gérées.</p>
        </div>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div class="p-6 bg-gradient-to-br from-primary-600 to-indigo-600 rounded-2xl shadow-premium text-white relative overflow-hidden group">
            ...
            <span class="block text-xs font-bold uppercase tracking-widest text-primary-200">Budget Total Dépensé</span>
            <span class="block text-3xl font-black mt-2 tracking-tight">{{ number_format($depenses->sum('montant'), 2) }} DH</span>
            <span class="block text-xs text-primary-100/80 mt-2 font-medium">Sur vos {{ $depenses->count() }} opérations</span>
        </div>
        ...
        <div class="p-6 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center gap-x-4">
            ...
            <div>
                <span class="block text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">Mes Immeubles Gérés</span>
        ...
        <div class="p-6 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center gap-x-4">
            ...
            <div>
                <span class="block text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">Justificatifs Fournis</span>
                ...
                <div class="p-6">
                    <form action="{{ route('syndic.depenses.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600 dark:text-slate-400 justify-center">
                                            <label for="justificatif-upload-syndic" class="relative cursor-pointer rounded-md font-semibold text-primary-600 hover:text-primary-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary-500">
                                                ...
                                                <input id="justificatif-upload-syndic" name="justificatif" type="file" class="sr-only" onchange="document.getElementById('fileNameSpanSyndicDep').innerText = this.files[0] ? this.files[0].name : 'Aucun fichier sélectionné'">
                                            </label>
                                        </div>
                                        ...
                                        <div id="fileNameDisplaySyndicDep" class="mt-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 py-1 px-2 rounded-lg inline-block">
                                            <span id="fileNameSpanSyndicDep">Aucun fichier sélectionné</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 pt-4">
                            <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-modal-add-depense-syndic">Annuler</button>
