...
<div class="space-y-8" x-data="adminDashboard">
    ...
    <div class="bg-amber-500/10 border border-amber-500/20 text-sm text-amber-600 dark:text-amber-400 rounded-2xl p-4 shadow-premium backdrop-blur-md flex items-start gap-x-3" role="alert">
        ...
        <div>
            <span class="font-bold">Attention :</span> {{ $stats['paiements_retard'] }} paiements de cotisations sont actuellement en retard ce mois-ci.
        </div>
    </div>
    @endif
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="flex items-center gap-x-3">
            ...
            <div class="flex items-center gap-x-2 py-2 px-3 rounded-xl bg-slate-900/60 border border-slate-800/60 text-xs font-semibold text-slate-400">
                <span class="size-1.5 inline-block bg-emerald-400 rounded-full animate-ping"></span>
                Actualisation dans <span class="text-emerald-400 font-bold tabular-nums" x-text="countdown + 's'"></span>
            </div>
    ...
    <div class="grid sm:grid-cols-2 gap-4 sm:gap-6">
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center justify-between group hover:shadow-premium-hover transition-all duration-300">
            <div class="space-y-1.5">
                <span class="text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">Actions Aujourd'hui</span>
                <div class="flex items-baseline gap-x-2">
                    <span class="text-3xl font-extrabold text-emerald-400 leading-none">{{ $stats['logs_today'] }}</span>
                    <span class="text-xs font-semibold text-slate-400">événements</span>
                </div>
                <p class="text-[11px] text-slate-400 dark:text-neutral-500">
                    <span class="font-bold text-slate-600 dark:text-neutral-300">{{ $stats['logs_last_hour'] }}</span> dans la dernière heure
                </p>
            </div>
        ...
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center justify-between group hover:shadow-premium-hover transition-all duration-300">
            <div class="space-y-1.5">
                <span class="text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">Dernière Action</span>
                @if($recentActivity->first())
                    ...
                    <p class="text-[11px] text-slate-400 dark:text-neutral-500">
                        <span class="font-bold text-blue-400">{{ $recentActivity->first()->action }}</span>
                        · {{ $recentActivity->first()->created_at->diffForHumans() }}
                    </p>
                @else
                    <span class="text-sm text-slate-400">Aucune activité</span>
                @endif
            </div>
    ...
    <div class="p-6 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h4 class="text-sm font-bold text-slate-800 dark:text-white">Activité des 7 derniers jours</h4>
                <p class="text-xs text-slate-400 mt-0.5">Nombre d'événements enregistrés par jour</p>
            </div>
            <span class="inline-flex items-center gap-x-1.5 py-1 px-2.5 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400">
                <i data-lucide="bar-chart-2" class="size-3"></i> 7 jours
            </span>
        </div>
            ...
            @foreach($activityByDay as $day)
                ...
                <div class="flex-1 flex flex-col items-center justify-end gap-y-1 h-full group/bar">
                    ...
                    <div class="w-full rounded-t-md transition-all duration-500 cursor-default
                         ...
                         title="{{ $day['count'] }} actions le {{ $day['date'] }}">
                    </div>
                    {{-- Day label --}}
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="p-6 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60">
            <div class="flex items-center justify-between mb-5">
                <h4 class="text-sm font-bold text-slate-800 dark:text-white">Actions les plus fréquentes</h4>
        ...
        <div class="p-6 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60">
            <div class="flex items-center justify-between mb-5">
                <h4 class="text-sm font-bold text-slate-800 dark:text-white">Utilisateurs les plus actifs</h4>
    ...
    <div class="flex flex-col border border-gray-200/60 dark:border-slate-800/60 rounded-2xl shadow-premium bg-white dark:bg-[#0D121F] overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/30 flex items-center justify-between">
            ...
            <div class="flex items-center gap-x-3">
                ...
                <a href="{{ route('admin.logs') }}" class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-lg text-xs font-semibold text-primary-400 hover:text-primary-300 hover:bg-primary-500/10 transition-all duration-200">
                    Voir tout <i data-lucide="arrow-right" class="size-3"></i>
                </a>
            </div>
        </div>
