...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Supervision des Paiements</h2>
        <p class="text-sm text-slate-500 dark:text-neutral-400">Suivi global des flux financiers, statistiques et validation des données.</p>
    </div>
</div>
    ...
    <div class="grid sm:grid-cols-3 gap-6 mb-8">
        ...
        <div class="relative overflow-hidden bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 rounded-2xl p-6 shadow-premium transition-all hover:scale-[1.01] duration-300">
            ...
            <div class="flex items-center gap-x-4">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 dark:text-neutral-400 uppercase tracking-widest">Collecté (Validé)</p>
                    <h3 class="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400 tracking-tight mt-0.5">{{ number_format($stats['totalCollecte'], 2) }} DH</h3>
                </div>
            </div>
        </div>
        ...
        <div class="relative overflow-hidden bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 rounded-2xl p-6 shadow-premium transition-all hover:scale-[1.01] duration-300">
            ...
            <div class="flex items-center gap-x-4">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 dark:text-neutral-400 uppercase tracking-widest">En attente</p>
                    <h3 class="text-2xl font-extrabold text-amber-600 dark:text-amber-505 mt-0.5">{{ number_format($stats['totalAttente'], 2) }} DH</h3>
                </div>
            </div>
        </div>
        ...
        <div class="relative overflow-hidden bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 rounded-2xl p-6 shadow-premium transition-all hover:scale-[1.01] duration-300">
            ...
            <div class="flex items-center gap-x-4">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 dark:text-neutral-400 uppercase tracking-widest">Total Transactions</p>
    ...
    <div class="flex flex-col border border-gray-200/60 dark:border-slate-800/60 rounded-2xl shadow-premium bg-white dark:bg-[#0D121F] overflow-hidden">
        ...
        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/30">
            <div class="sm:col-span-1 max-w-sm w-full relative">
                <input x-model="search" type="text" class="py-2.5 px-4 ps-11 block w-full border-gray-200 dark:border-slate-850 dark:bg-[#080B11] dark:text-slate-300 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Rechercher résident...">
            ...
            <div class="sm:col-span-2 md:grow flex justify-end gap-x-2">
                ...
                <div class="relative inline-flex">
                    ...
                    <div x-show="showStat" class="absolute right-0 top-full z-[100] mt-2 w-48 bg-white border border-gray-200 shadow-xl rounded-2xl p-1.5 dark:bg-neutral-900 dark:border-neutral-800" style="display: none;"
                        ...
                        <div @click="statutSelectionne = 'validé'; showStat = false" class="cursor-pointer flex items-center py-2 px-3 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800/60">Payé (Validé)</div>
                        <div @click="statutSelectionne = 'en attente'; showStat = false" class="cursor-pointer flex items-center py-2 px-3 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800/60">En attente</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-250 dark:divide-slate-800">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr>
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">Résident</th>
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">Immeuble & Appt</th>
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">Montant</th>
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">Date</th>
                ...
                <tbody class="divide-y divide-gray-200/60 dark:divide-slate-800/60">
                    ...
                    <tr x-show="matches('{{ $residentName }}', '{{ $immeubleName }}', '{{ $paiement->statut }}')" class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-neutral-400">{{ $immeubleName }} - Appt {{ $appt?->numero }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-850 dark:text-white">{{ number_format($paiement->montant, 2) }} DH</td>
