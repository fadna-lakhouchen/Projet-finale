...
@section('content')
<div x-data="adminLogs({{ json_encode($logs->map(function($log) {
        return [
            'id' => $log->id,
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">Logs Système & Journal d'Audit</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">
                Visualisez et suivez l'historique complet de toutes les actions de création, modification et suppression pour une sécurité et traçabilité absolue.
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium hover:shadow-premium-hover transition-all duration-300 dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center gap-x-4">
            ...
            <div>
                <span class="block text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">Ajouts</span>
        ...
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium hover:shadow-premium-hover transition-all duration-300 dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center gap-x-4">
            ...
            <div>
                <span class="block text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">Modifications</span>
        ...
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium hover:shadow-premium-hover transition-all duration-300 dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center gap-x-4">
            ...
            <div>
                <span class="block text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">Suppressions</span>
    ...
    <!-- Table Container (With same fixes applied to avoid dropdown clipping) -->
    <div class="flex flex-col bg-white/85 dark:bg-[#0D121F]/90 border border-gray-200/60 dark:border-slate-800/60 rounded-2xl shadow-premium backdrop-blur-md">
        ...
        <div class="px-6 py-5 grid gap-4 md:flex md:justify-between md:items-center border-b border-gray-200/60 dark:border-slate-800/60 bg-white/40 dark:bg-[#0D121F]/40">
            ...
            <div class="sm:col-span-1 max-w-sm w-full relative">
                <input x-model="search" type="text" class="py-2.5 px-4 ps-11 block w-full border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 dark:placeholder-neutral-500 transition-all duration-200" placeholder="Rechercher par utilisateur, table...">
            ...
            <div class="flex items-center gap-3 relative">
                ...
                <div class="relative" @click.outside="openAction = false">
                    <button @click="openAction = !openAction" type="button" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-gray-200 bg-white/50 hover:bg-gray-50 dark:bg-[#090D16]/50 dark:border-slate-800 dark:text-white dark:hover:bg-slate-900/50 shadow-sm transition-all duration-200">
                        <span x-text="actionSelectionnee === 'all' ? 'Toutes les actions' : (actionSelectionnee === 'created' ? 'Ajout' : (actionSelectionnee === 'updated' ? 'Modification' : 'Suppression'))" class="truncate max-w-[150px]"></span>
                        <i data-lucide="chevron-down" class="size-4 text-gray-400 transition-transform duration-200" :class="{'rotate-180': openAction}"></i>
                    </button>
                    <div x-show="openAction" x-cloak class="absolute right-0 top-full z-[100] mt-2 w-48 bg-white/95 dark:bg-[#0D121F]/95 border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md" style="display: none;">
                        <div @click="actionSelectionnee = 'all'; openAction = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">Toutes les actions</div>
                        <div @click="actionSelectionnee = 'created'; openAction = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">Ajout</div>
                        <div @click="actionSelectionnee = 'updated'; openAction = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">Modification</div>
                        <div @click="actionSelectionnee = 'deleted'; openAction = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">Suppression</div>
                    </div>
                </div>
                ...
                <div class="relative" @click.outside="openDate = false">
                    <button @click="openDate = !openDate" type="button" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-gray-200 bg-white/50 hover:bg-gray-50 dark:bg-[#090D16]/50 dark:border-slate-800 dark:text-white dark:hover:bg-slate-900/50 shadow-sm transition-all duration-200">
                        <span x-text="dateSelectionnee === 'all' ? 'Toutes les dates' : (dateSelectionnee === 'today' ? 'Aujourd\'hui' : '7 derniers jours')" class="truncate max-w-[150px]"></span>
                        <i data-lucide="chevron-down" class="size-4 text-gray-400 transition-transform duration-200" :class="{'rotate-180': openDate}"></i>
                    </button>
                    <div x-show="openDate" x-cloak class="absolute right-0 top-full z-[100] mt-2 w-48 bg-white/95 dark:bg-[#0D121F]/95 border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md" style="display: none;">
                        <div @click="dateSelectionnee = 'all'; openDate = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">Toutes les dates</div>
                        <div @click="dateSelectionnee = 'today'; openDate = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">Aujourd'hui</div>
                        <div @click="dateSelectionnee = 'week'; openDate = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">7 derniers jours</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-x-auto rounded-b-2xl overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200/60 dark:divide-slate-800/60">
                <thead class="bg-gray-50/50 dark:bg-[#090D16]/40">
                    <tr>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">Utilisateur</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">Action</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">Modèle / Cible</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">ID Cible</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">Date & Heure</th>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-x-3">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider mt-0.5" 
                                              x-text="log.user_role === 'administrateur' ? 'Super Admin' : (log.user_role === 'syndic' ? 'Syndic' : 'Résident')"></span>
                                    </div>
                                </div>
                            </td>
                    ...
                    <tr x-show="filteredLogs.length === 0">
                        <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-2">
                                <i data-lucide="scroll" class="size-10 text-gray-300 dark:text-slate-700 animate-pulse"></i>
                                <span class="font-medium">Aucun log système ne correspond à vos filtres de recherche.</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
    ...
    <div x-show="showModal" class="fixed inset-0 z-[100] overflow-y-auto bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4" x-cloak style="display: none;" @keydown.escape.window="showModal = false">
        <div class="bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col dark:bg-[#0D121F]" @click.outside="showModal = false">
            ...
            <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
                <div>
                    <h3 class="font-bold text-gray-800 dark:text-white text-lg">Détails de l'Action</h3>
            ...
            <div class="p-6 overflow-y-auto space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4 p-4 bg-gray-50 border border-gray-150 rounded-xl dark:bg-neutral-900/50 dark:border-neutral-800/60">
                    ...
                    <div>
                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Type d'élément</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col bg-rose-500/5 border border-rose-100/50 rounded-xl p-4 dark:bg-rose-950/5 dark:border-rose-900/20">
                            ...
                            <div class="space-y-1.5 flex-1 min-h-[80px]">
                                ...
                                <template x-if="!selectedLog || !selectedLog.modifications || !selectedLog.modifications.before">
                                    <span class="text-xs text-slate-400 italic block text-center mt-6">Aucune valeur précédente (Nouvel élément)</span>
                                </template>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col bg-emerald-500/5 border border-emerald-100/50 rounded-xl p-4 dark:bg-emerald-950/5 dark:border-emerald-900/20">
                            ...
                            <div class="space-y-1.5 flex-1 min-h-[80px]">
                                ...
                                <template x-if="!selectedLog || !selectedLog.modifications || !selectedLog.modifications.after">
                                    <span class="text-xs text-slate-400 italic block text-center mt-6">Aucune modification enregistrée</span>
                                </template>
                            </div>
                        </div>
                    </div>
