...
<div x-data="adminImmeubles">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Supervision des Immeubles</h2>
            <p class="text-sm text-slate-500 dark:text-neutral-400">Vue d'ensemble et administration de tous les bâtiments enregistrés.</p>
        </div>
    ...
    <div class="flex flex-col border border-gray-200/60 dark:border-slate-800/60 rounded-2xl shadow-premium bg-white dark:bg-[#0D121F] overflow-hidden">
        ...
        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/30">
            <div class="sm:col-span-1 max-w-sm w-full relative">
                <input x-model="search" type="text" class="py-2.5 px-4 ps-11 block w-full border-gray-200 dark:border-slate-850 dark:bg-[#080B11] dark:text-slate-300 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Rechercher par nom ou adresse...">
            ...
            <div class="sm:col-span-2 md:grow flex justify-end gap-x-2">
                ...
                <div class="relative inline-flex">
                    ...
                    <div x-show="showSyndic" class="absolute right-0 top-full z-[100] mt-2 w-60 bg-white border border-gray-200 shadow-xl rounded-2xl p-1.5 dark:bg-neutral-900 dark:border-neutral-800" style="display: none;"
                        ...
                        <div @click="filterSyndic = 'all'; showSyndic = false" class="cursor-pointer flex items-center py-2 px-3 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800/60">Tous les syndics</div>
                ...
                <div class="relative inline-flex">
                    ...
                    <div x-show="showStatut" class="absolute right-0 top-full z-[100] mt-2 w-48 bg-white border border-gray-200 shadow-xl rounded-2xl p-1.5 dark:bg-neutral-900 dark:border-neutral-800" style="display: none;"
                         ...
                         x-transition:enter-start="transform opacity-0 scale-95"
                         x-transition:enter-end="transform opacity-100 scale-100">
                        <div @click="filterStatut = 'all'; showStatut = false" class="cursor-pointer flex items-center py-2 px-3 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800/60">Tous les statuts</div>
                        <div @click="filterStatut = 'Sain'; showStatut = false" class="cursor-pointer flex items-center py-2 px-3 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800/60">Sain</div>
                        <div @click="filterStatut = 'En travaux'; showStatut = false" class="cursor-pointer flex items-center py-2 px-3 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800/60">En travaux</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-250 dark:divide-slate-800">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">Syndic Responsable</th>
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">Logements</th>
                    ...
                    <tr x-show="matches('{{ addslashes($immeuble->nom) }}', '{{ addslashes($immeuble->adresse) }}', '{{ addslashes($immeuble->syndic ? $immeuble->syndic->prenom . ' ' . $immeuble->syndic->nom : 'N/A') }}', 'Sain')" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-slate-700 dark:text-neutral-300">{{ $immeuble->nombre_appartements }} Appartements</span>
                                <span class="text-[10px] text-slate-400 font-semibold mt-0.5">{{ $immeuble->nombre_etages }} Étages</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <form :action="isEditing ? `/admin/immeubles/${immeubleEnCours.id}` : '{{ route('admin.immeubles.store') }}'" method="POST">
                        ...
                        <div class="grid gap-y-5">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-white">Nombre d'appartements</label>
                        ...
                        <div class="flex justify-end items-center gap-x-3.5 mt-8 border-t border-slate-100 dark:border-slate-800 pt-5">
                            <button type="button" class="py-2.5 px-4 text-sm font-semibold border rounded-xl border-slate-200 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800 dark:text-white transition-colors" data-hs-overlay="#hs-modal-add-immeuble">Annuler</button>
