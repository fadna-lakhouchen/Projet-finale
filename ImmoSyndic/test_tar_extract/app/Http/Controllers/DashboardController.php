...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        return match($role) {
            'administrateur', 'admin' => redirect()->route('admin.dashboard'),
    ...
    /**
     * Basculer le rôle actif de l'utilisateur en session.
     */
    public function switchRole($role)
    {
        $user = Auth::user();

        // Validation que l'utilisateur possède réellement le rôle demandé
        if (in_array($role, ['syndic', 'resident', 'administrateur', 'admin'])) {
            if ($user->hasRole($role) || $user->role === $role || ($role === 'administrateur' && $user->role === 'admin')) {
                session(['active_role' => $role]);
                return redirect()->route('dashboard')->with('success', __('Espace mis à jour avec succès.'));
            }
        }
    ...
    public function adminDashboard()
    {
        ...
        $stats = [
            'total_residents'          => User::where('role', 'resident')->orWhereHas('roles', fn($q) => $q->where('name', 'resident'))->count(),
    ...
    public function adminImmeubles()
    {
        ...
        $syndics = User::where('role', 'syndic')->orWhereHas('roles', fn($q) => $q->where('name', 'syndic'))->get();
    ...
    {
        $residents = User::where('role', 'resident')->orWhereHas('roles', fn($q) => $q->where('name', 'resident'))->with('appartements.immeuble')->get();
    ...
    public function syndicDashboard()
    {
        ...
        foreach ($paiements as $p) {
            ...
            $userName = $p->user ? $p->user->name : __('Résident');
            
            $activites->push([
                'type' => __('Paiement'),
                'evenement' => __('Paiement reçu'),
                'concerne' => $appartement ? "$immeubleNom - " . __('Appt') . " " . $appartement->numero : $immeubleNom,
                'details' => number_format($p->montant, 2) . " " . __('DH') . " (" . __('Par') . " " . $userName . ")",
                'date' => $p->created_at,
                'icon' => 'check',
                'color' => 'emerald',
                'bg_row' => ''
        ...
        foreach ($incidents as $i) {
            ...
            $userName = $i->user ? $i->user->name : __('Syndic (Admin)');
            
            $activites->push([
                'type' => __('Signalement'),
                'evenement' => $i->cree_par_admin ? __('Signalement (Admin)') : __('Signalement Résident'),
                'concerne' => $i->appartement_id ? "$immeubleNom - " . __('Appt') . " " . ($i->appartement->numero ?? '') : $immeubleNom,
                'details' => $i->titre . " (" . __('Par') . " " . $userName . ")",
                'date' => $i->created_at,
    ...
    public function syndicResidents()
    {
        ...
        $residents = User::where(function($query) {
    ...
    public function residentDashboard()
    {
        ...
        foreach ($mesPaiements as $p) {
            $activites->push([
                ...
                'type' => __('Paiement'),
                'description' => __('Règlement de charge REF-') . str_pad($p->id, 6, '0', STR_PAD_LEFT),
                'statut' => __($p->statut),
    ...
    public function signalReadyToPay(Request $request)
    {
        ...
        if ($syndic) {
            $dateFr = ucfirst(\Carbon\Carbon::parse($charge->date_echeance)->locale('fr')->translatedFormat('F Y'));
    ...
    public function syndicSecondarySyndics()
    {
        ...
        $secondarySyndics = User::where(function($query) {
                $query->where('role', 'syndic')
                      ->orWhereHas('roles', fn($q) => $q->where('name', 'syndic'));
            })
    ...
    public function syndicLogs()
    {
        ...
        $secondarySyndicIds = User::where(function($query) {
                $query->where('role', 'syndic')
                      ->orWhereHas('roles', fn($q) => $q->where('name', 'syndic'));
            })
            ->whereHas('secondaryImmeubles', function($q) use ($immeubleIds) {
