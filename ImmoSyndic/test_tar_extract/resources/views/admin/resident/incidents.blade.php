...
<section class="space-y-6">
    <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">{{ __('Nouveau Signalement') }}</h2>
        ...
        <form id="incident-form" action="{{ route('resident.incidents.store') }}" method="POST"
            ...
            <div class="grid gap-y-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium mb-2 dark:text-white">{{ __('Titre du problème') }} <span class="text-red-500">*</span></label>
                        <input type="text" name="titre" required
                            ...
                            placeholder="{{ __('Titre du problème') }}...">
                    </div>

                    {{-- Priorité dropdown --}}
                    <div>
                        <label class="block text-sm font-medium mb-2 dark:text-white">{{ __('Priorité') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="relative">
                            ...
                            <div x-show="openPrio" x-cloak
                                ...
                                <div @click="setPrio('basse', '{{ __('Basse') }}', 'gray')"
                ...
                <div>
                    <label class="block text-sm font-medium mb-2 dark:text-white">{{ __('Description détaillée') }} <span class="text-red-500">*</span></label>
                    <textarea name="description" rows="4" required
                        class="py-3 px-4 block w-full border-gray-200 rounded-lg border text-sm dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 focus:border-primary-500 focus:ring-primary-500"
                        placeholder="{{ __('Décrivez le problème...') }}"></textarea>
                </div>
                ...
                <div class="pt-2">
                    <button type="submit" class="py-3 px-6 bg-primary-600 text-white rounded-lg text-sm font-semibold hover:bg-primary-700 flex items-center gap-2 transition-colors">
                        <i data-lucide="send" class="size-4"></i> {{ __('Envoyer') }}
                    </button>
                </div>
            </div>
        </form>
    ...
    <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6 mt-12">{{ __('Historique de mes signalements') }}</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                <thead class="bg-gray-50 dark:bg-neutral-800">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">{{ __('Problème') }}</th>
                        <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">{{ __('Priorité') }}</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            @php
                                $prioRaw = $incident->getRawOriginal('priorite');
                                $prioColor = match($prioRaw) {
                                    'urgente' => 'red',
                                    'haute'   => 'orange',
                                    'moyenne' => 'blue',
                                    default   => 'gray'
                                };
                                $prioLabel = match($prioRaw) {
                                    'urgente' => __('Urgente'),
                                    'haute'   => __('Haute'),
                                    'moyenne' => __('Moyenne'),
                                    default   => __('Basse')
                            ...
                            @endphp
                            <span class="inline-flex items-center gap-1.5 py-1 px-2 rounded-md text-xs font-medium bg-{{ $prioColor }}-100 text-{{ $prioColor }}-800">
                                {{ $prioLabel }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            @php
                                $statutRaw = $incident->getRawOriginal('statut');
                                $statutColor = match(strtolower($statutRaw)) {
                                    'nouveau', 'ouvert', 'à traiter' => 'blue',
                                    'en cours'                       => 'orange',
                                    'résolu', 'terminé'              => 'green',
                                    default                          => 'gray'
                                };
                                $statutLabel = match(strtolower($statutRaw)) {
                                    'nouveau'   => __('Nouveau'),
                                    'ouvert'    => __('Ouvert'),
                                    'à traiter' => __('À traiter'),
                                    'en cours'  => __('En cours'),
                                    'résolu'    => __('Résolu'),
                                    'terminé'   => __('Terminé'),
                                    default     => ucfirst($statutRaw)
                                };
                            @endphp
                            <span class="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-full text-xs font-medium bg-{{ $statutColor }}-100 text-{{ $statutColor }}-800 dark:bg-{{ $statutColor }}-900/30 dark:text-{{ $statutColor }}-500">
                                {{ $statutLabel }}
                            </span>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-gray-500 dark:text-neutral-400">{{ __('Vous n\'avez pas encore déclaré d\'incident.') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
