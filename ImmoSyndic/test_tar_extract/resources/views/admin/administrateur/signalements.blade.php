...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-850 dark:text-white">{{ __('Signalements &amp; Incidents') }}</h2>
            <p class="text-sm text-slate-550 dark:text-neutral-400">{{ __('Suivi des réclamations et des interventions techniques.') }}</p>
        </div>
    </div>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium hover:shadow-premium-hover transition-all duration-300 dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center justify-between group">
            <div class="space-y-2">
                <span class="text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">{{ __('Ouverts') }}</span>
        ...
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium hover:shadow-premium-hover transition-all duration-300 dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center justify-between group">
            <div class="space-y-2">
                <span class="text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">{{ __('En Cours') }}</span>
                <div class="flex items-baseline gap-x-2">
        ...
        <div class="p-5 bg-white border border-gray-200/60 rounded-2xl shadow-premium hover:shadow-premium-hover transition-all duration-300 dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center justify-between group">
            <div class="space-y-2">
                <span class="text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">{{ __('Résolus (Total)') }}</span>
                <div class="flex items-baseline gap-x-2">
    ...
    <div class="flex flex-col border border-gray-200/60 dark:border-slate-800/60 rounded-2xl shadow-premium bg-white dark:bg-[#0D121F] overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/30 flex items-center justify-between">
            <h4 class="text-sm font-bold text-slate-800 dark:text-white">{{ __('Liste des Signalements') }}</h4>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-250 dark:divide-slate-800">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr>
                        <th class="px-6 py-3.5 text-start text-xs font-semibold text-slate-400 uppercase dark:text-neutral-450">{{ __('Incident') }}</th>
                        <th class="px-6 py-3.5 text-start text-xs font-semibold text-slate-400 uppercase dark:text-neutral-450">{{ __('Priorité') }}</th>
                        <th class="px-6 py-3.5 text-start text-xs font-semibold text-slate-400 uppercase dark:text-neutral-450">{{ __('Rapporté par') }}</th>
                        <th class="px-6 py-3.5 text-start text-xs font-semibold text-slate-400 uppercase dark:text-neutral-450">{{ __('Statut') }}</th>
                        <th class="px-6 py-3.5 text-end text-xs font-semibold text-slate-400 uppercase dark:text-neutral-450">{{ __('Date') }}</th>
                    </tr>
                </thead>
