...
<div x-data="adminLogs({{ json_encode($logs->map(function($log) {
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">{{ __('Logs Système & Journal d\'Audit') }}</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">
                {{ __('Visualisez et suivez l\'historique complet de toutes les actions de création, modification et suppression pour une sécurité et traçabilité absolue.') }}
            </p>
        </div>
    </div>
                    ...
                    <tr x-show="filteredLogs.length === 0">
                        <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <span class="font-medium">{{ __('Aucun log système ne correspond à vos filtres de recherche.') }}</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
