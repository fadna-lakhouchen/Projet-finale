...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-850 dark:text-white">{{ __('Rapports & Statistiques') }}</h2>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white border border-gray-200/60 rounded-2xl shadow-premium p-6 dark:bg-[#0D121F] dark:border-slate-800/60 h-80 flex flex-col hover:shadow-premium-hover transition-all duration-300">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-base font-bold text-slate-800 dark:text-white">{{ __('Évolution des Recettes') }}</h3>
            ...
            <div class="mt-4 flex justify-between text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                <span>{{ __('Jan') }}</span><span>{{ __('Mar') }}</span><span>{{ __('Mai') }}</span><span>{{ __('Jul') }}</span><span>{{ __('Sep') }}</span><span>{{ __('Nov') }}</span>
            </div>
        </div>
        
        <!-- Expenses Breakdown Chart -->
        <div class="bg-white border border-gray-200/60 rounded-2xl shadow-premium p-6 dark:bg-[#0D121F] dark:border-slate-800/60 h-80 flex flex-col hover:shadow-premium-hover transition-all duration-300">
            <h3 class="text-base font-bold text-slate-800 dark:text-white mb-4">{{ __('Répartition des Dépenses') }}</h3>
            <div class="flex-grow flex items-center justify-center relative">
                <div class="relative size-44">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center flex-col">
                        ...
                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">{{ __('Maintenance') }}</span>
                    </div>
                </div>
            </div>
