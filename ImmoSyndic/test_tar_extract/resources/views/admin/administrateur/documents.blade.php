...
<div x-data="adminDocuments" class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">{{ __('Coffre-fort Documentaire') }}</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ __('Gérez l\'ensemble des documents administratifs, financiers et légaux de toutes les copropriétés.') }}</p>
        </div>
    ...
    <div class="bg-white/80 dark:bg-[#0D121F]/90 border border-gray-200/60 rounded-2xl shadow-premium p-6 dark:border-slate-800/60 backdrop-blur-md">
        <div class="flex justify-between items-center mb-3">
            <h4 class="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-x-2">
                ...
                {{ __('Espace Disque Utilisé (Coffre-fort global)') }}
            </h4>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div @click="categorieSelectionne = categorieSelectionne === 'Facture' ? 'all' : 'Facture'" 
            ...
            <div class="flex items-center gap-x-4">
                ...
                <div>
                    <span class="block text-sm font-bold text-slate-800 dark:text-white">{{ __('Factures') }}</span>
                    <span class="block text-xs font-semibold text-slate-400 dark:text-neutral-450 mt-0.5">{{ $documents->where('categorie', 'Facture')->count() }} {{ __('fichier(s)') }}</span>
                </div>
            </div>
        </div>
        ...
        <div @click="categorieSelectionne = categorieSelectionne === 'Contrat' ? 'all' : 'Contrat'" 
            ...
            <div class="flex items-center gap-x-4">
                ...
                <div>
                    <span class="block text-sm font-bold text-slate-800 dark:text-white">{{ __('Contrat & Devis') }}</span>
                    <span class="block text-xs font-semibold text-slate-400 dark:text-neutral-450 mt-0.5">{{ $documents->where('categorie', 'Contrat')->count() }} {{ __('fichier(s)') }}</span>
                </div>
            </div>
        </div>
        ...
        <div @click="categorieSelectionne = categorieSelectionne === 'PV' ? 'all' : 'PV'" 
            ...
            <div class="flex items-center gap-x-4">
                ...
                <div>
                    <span class="block text-sm font-bold text-slate-800 dark:text-white">{{ __('Rapports de Réunions') }}</span>
                    <span class="block text-xs font-semibold text-slate-400 dark:text-neutral-450 mt-0.5">{{ $documents->where('categorie', 'PV')->count() }} {{ __('fichier(s)') }}</span>
                </div>
            </div>
        </div>
        ...
        <div @click="categorieSelectionne = categorieSelectionne === 'Autre' ? 'all' : 'Autre'" 
            ...
            <div class="flex items-center gap-x-4">
                ...
                <div>
                    <span class="block text-sm font-bold text-slate-800 dark:text-white">{{ __('Autres') }}</span>
                    <span class="block text-xs font-semibold text-slate-400 dark:text-neutral-450 mt-0.5">{{ $documents->where('categorie', 'Autre')->count() }} {{ __('fichier(s)') }}</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="flex flex-wrap items-center gap-3 relative">
                ...
                <div class="relative">
                    <button @click="openCat = !openCat" @click.outside="openCat = false" type="button" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-gray-200 bg-white/50 hover:bg-gray-50 dark:bg-[#090D16]/50 dark:border-slate-800 dark:text-white dark:hover:bg-slate-900/50 shadow-sm transition-all duration-200">
                        <span x-text="categorieSelectionne === 'all' ? '{{ __('Filtrer par Catégorie') }}' : (categorieSelectionne === 'PV' ? '{{ __('Rapport de Réunion (PV)') }}' : (categorieSelectionne === 'Facture' ? '{{ __('Facture') }}' : (categorieSelectionne === 'Contrat' ? '{{ __('Contrat & Devis') }}' : '{{ __('Autres') }}')))" class="truncate max-w-[150px]"></span>
                    ...
                    <div x-show="openCat" x-cloak class="absolute right-0 top-full z-[100] mt-2 w-56 bg-white/95 dark:bg-[#0D121F]/95 border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md" style="display: none;">
                        <div @click="categorieSelectionne = 'all'; openCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Toutes les catégories') }}</div>
                        <div @click="categorieSelectionne = 'Facture'; openCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Facture') }}</div>
                        <div @click="categorieSelectionne = 'Contrat'; openCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Contrat & Devis') }}</div>
                        <div @click="categorieSelectionne = 'PV'; openCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Rapport de Réunion (PV)') }}</div>
                        <div @click="categorieSelectionne = 'Autre'; openCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Autre') }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-x-auto rounded-b-2xl overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200/60 dark:divide-slate-800/60">
                <thead class="bg-gray-50/50 dark:bg-[#090D16]/40">
                    <tr>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Nom du Fichier') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Catégorie') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Immeuble') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Taille') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Date d\'ajout') }}</th>
                    ...
                    <tr x-show="matches('{{ addslashes($doc->titre) }}', '{{ addslashes($immeubleNom) }}', '{{ $doc->categorie }}')" class="hover:bg-gray-50/50 dark:hover:bg-slate-900/30 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center gap-x-1.5 py-1 px-2.5 rounded-full text-xs font-semibold bg-{{ $color }}-50 text-{{ $color }}-600 dark:bg-{{ $color }}-950/40 dark:text-{{ $color }}-400 border border-{{ $color }}-100 dark:border-{{ $color }}-900/20">
                                {{ $doc->categorie === 'PV' ? __('Rapport de Réunion (PV)') : __($doc->categorie) }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <div class="inline-flex items-center gap-x-2">
                                <!-- Download -->
                                <a href="{{ $doc->url }}" target="_blank" download class="py-1.5 px-1.5 text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-950/30 rounded-lg border border-gray-200/80 hover:border-primary-300 dark:border-slate-800/80 dark:hover:border-primary-900/30 transition-all duration-200" title="{{ __('Télécharger') }}">
                                    <i data-lucide="download" class="size-4"></i>
                                </a>
                                
                                <!-- Delete -->
                                <form action="{{ route('admin.documents.destroy', $doc->id) }}" method="POST" onsubmit="return confirm('{{ __('Voulez-vous vraiment supprimer définitivement ce document ?') }}');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="py-1.5 px-1.5 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-lg border border-gray-200/80 hover:border-rose-300 dark:border-slate-800/80 dark:hover:border-rose-900/30 transition-all duration-200" title="{{ __('Supprimer') }}">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <span class="font-medium">{{ __('Aucun document stocké dans le coffre-fort pour le moment.') }}</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
            <div class="w-full flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F]">
                ...
                <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
                    <h3 class="font-bold text-gray-800 dark:text-white text-lg">{{ __('Déposer un Document') }}</h3>
                ...
                <div class="p-6">
                    <form action="{{ route('admin.documents.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid gap-y-4">
                            <!-- Titre -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Titre du document') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Immeuble') }}</label>
                                <div class="relative" x-data="{ open: false }">
                                    <button @click="open = !open" @click.outside="open = false" type="button" class="py-2.5 px-4 flex justify-between items-center w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/85 dark:text-neutral-300 transition-all duration-200 text-left">
                                        <span x-text="documentEnCours.immeuble_id ? ({ @foreach($immeubles as $imm) '{{ $imm->id }}': '{{ addslashes($imm->nom) }}', @endforeach }[documentEnCours.immeuble_id] || '{{ __('Sélectionner un immeuble') }}') : '{{ __('Sélectionner un immeuble') }}'"></span>
                            ...
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Catégorie') }}</label>
                                <div class="relative" x-data="{ open: false }">
                                    <button @click="open = !open" @click.outside="open = false" type="button" class="py-2.5 px-4 flex justify-between items-center w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/85 dark:text-neutral-300 transition-all duration-200 text-left">
                                        <span x-text="documentEnCours.categorie ? (documentEnCours.categorie === 'PV' ? '{{ __('Rapport de Réunion (PV)') }}' : (documentEnCours.categorie === 'Facture' ? '{{ __('Facture') }}' : (documentEnCours.categorie === 'Contrat' ? '{{ __('Contrat & Devis') }}' : '{{ __('Autres') }}'))) : '{{ __('Sélectionner une catégorie') }}'"></span>
                                        <i data-lucide="chevron-down" class="size-4 text-gray-400 transition-transform duration-200" :class="{'rotate-180': open}"></i>
                                    </button>
                                    ...
                                    <div x-show="open" x-cloak class="absolute left-0 top-full z-[100] mt-2 w-full bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md" style="display: none;">
                                        <div @click="documentEnCours.categorie = 'Facture'; open = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Facture') }}</div>
                                        <div @click="documentEnCours.categorie = 'Contrat'; open = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Contrat & Devis') }}</div>
                                        <div @click="documentEnCours.categorie = 'PV'; open = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Rapport de Réunion (PV)') }}</div>
                                        <div @click="documentEnCours.categorie = 'Autre'; open = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Autre') }}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Fichier Upload -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Fichier (PDF, Image, Doc - Max 20MB)') }}</label>
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600 dark:text-slate-400 justify-center">
                                            <label for="file-upload" class="relative cursor-pointer rounded-md font-semibold text-primary-600 hover:text-primary-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary-500">
                                                ...
                                                <input id="file-upload" name="fichier" type="file" class="sr-only" required onchange="document.getElementById('fileNameSpan').innerText = this.files[0] ? this.files[0].name : '{{ __('Aucun fichier sélectionné') }}'">
                                            </label>
                                        </div>
                                        <p class="text-xs text-gray-500 dark:text-slate-500">{{ __('PDF, PNG, JPG, DOC, XLS jusqu\'à 20 Mo') }}</p>
                                        <div id="fileNameDisplay" class="mt-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 py-1 px-2 rounded-lg inline-block">
                                            <span id="fileNameSpan">{{ __('Aucun fichier sélectionné') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 pt-4">
                            <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-modal-upload-document">{{ __('Annuler') }}</button>
                            <button type="submit" class="py-2 px-4 text-sm font-semibold bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 text-white rounded-xl shadow-md shadow-primary-500/10 transition-all duration-300">{{ __('Valider & Déposer') }}</button>
                        </div>
                    </form>
                </div>
            </div>
