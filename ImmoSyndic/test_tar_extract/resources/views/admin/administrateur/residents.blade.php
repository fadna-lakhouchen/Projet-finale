...
<div x-data="{ 
    ...
    showStat: false,
    isEditing: false,
    residentEnCours: { id: '', prenom: '', nom: '', email: '', telephone: '', cin: '', notes: '', role: 'resident', immeuble_id: '', numero_appartement: '', date_entree: '', override_mois_retard: '', is_syndic: false },
    init() {
    ...
    initAjout() {
        ...
        this.residentEnCours = { id: '', prenom: '', nom: '', email: '', telephone: '', cin: '', notes: '', role: 'resident', immeuble_id: '', numero_appartement: '', date_entree: '', override_mois_retard: '', is_syndic: false };
        if (window.editor) window.editor.commands.setContent('');
    },
    initEdit(id, prenom, nom, email, telephone, cin, immeuble_id, numero_appt, date_e, notes, override_mois_retard, is_syndic) {
        this.isEditing = true;
        this.residentEnCours = { id: id, prenom: prenom, nom: nom, email: email, telephone: telephone, cin: cin, notes: notes, role: 'resident', immeuble_id: immeuble_id, numero_appartement: numero_appt, date_entree: date_e, override_mois_retard: override_mois_retard, is_syndic: is_syndic };
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">{{ __('Gestion des Résidents') }}</h2>
            ...
            <table class="min-w-full divide-y divide-gray-250 dark:divide-slate-800">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">{{ __('Logement') }}</th>
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">{{ __('État du Compte') }}</th>
                    ...
                    <tr x-show="matches('{{ $resident->prenom }} {{ $resident->nom }}', '{{ $immeubleName }}')" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <div class="inline-flex items-center gap-x-2">
                                <button @click="initEdit({{ json_encode($resident->id) }}, {{ json_encode($resident->prenom) }}, {{ json_encode($resident->nom) }}, {{ json_encode($resident->email) }}, {{ json_encode($resident->telephone) }}, {{ json_encode($resident->cin ?? '') }}, {{ json_encode($immeubleId) }}, {{ json_encode($appt ? $appt->numero : '') }}, {{ json_encode($appt ? $appt->pivot->date_entree : '') }}, {{ json_encode($resident->notes ?? '') }}, {{ json_encode($appt ? $appt->override_mois_retard : '') }}, {{ $resident->hasRole('syndic') ? 'true' : 'false' }})" type="button" data-hs-overlay="#hs-modal-add-resident" class="p-2 inline-flex items-center justify-center gap-x-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 dark:bg-slate-800/40 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800 transition-all">
                ...
                <div class="p-6 overflow-y-auto max-h-[75vh]">
                    <form :action="isEditing ? `/admin/residents/${residentEnCours.id}` : '{{ route('admin.residents.store') }}'" method="POST">
                        ...
                        <div class="grid gap-y-5">
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-white">{{ __('Email (Connexion)') }}</label>
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-white">{{ __('Surcharge mois de retard') }}</label>
                                    <input name="override_mois_retard" x-model="residentEnCours.override_mois_retard" type="number" min="0" class="py-2.5 px-4 block w-full border-gray-200 dark:border-slate-850 dark:bg-[#080B11] dark:text-slate-300 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="{{ __('Automatique (laisser vide)') }}">
                                </div>
                            </div>
                            
                            <!-- Est aussi Syndic Section -->
                            <div class="space-y-4 border border-gray-200/60 dark:border-slate-800/60 rounded-2xl p-4 bg-slate-50/[0.02]">
                                <div class="flex items-center">
                                    <input type="checkbox" name="is_syndic" value="1" x-model="residentEnCours.is_syndic" id="is_syndic" class="shrink-0 mt-0.5 border-gray-200 rounded-lg text-primary-600 focus:ring-primary-500 dark:bg-neutral-800 dark:border-neutral-700">
                                    <label for="is_syndic" class="text-sm font-semibold text-slate-700 ms-3 dark:text-neutral-400 cursor-pointer">
                                        {{ __('Est aussi Syndic (Gestionnaire)') }}
                                    </label>
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-end items-center gap-x-3 mt-8 border-t border-slate-100 dark:border-slate-800 pt-5">
                            <button type="button" class="py-2.5 px-4 text-sm font-semibold border rounded-xl border-slate-200 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800 dark:text-white transition-colors" data-hs-overlay="#hs-modal-add-resident">{{ __('Annuler') }}</button>
