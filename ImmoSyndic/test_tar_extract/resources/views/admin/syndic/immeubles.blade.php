...
<div x-data="syndicImmeubles({ syndicNames: { @foreach($syndics as $s) '{{ $s->id }}': '{{ addslashes($s->prenom) }} {{ addslashes($s->nom) }}', @endforeach } })">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">{{ __('Mes Immeubles') }}</h2>
            <p class="text-sm text-slate-500 dark:text-neutral-400">{{ __('Gérez et supervisez les bâtiments sous votre responsabilité directe.') }}</p>
        </div>
    ...
    <div class="mb-6 bg-rose-500/10 border border-rose-500/20 text-sm text-rose-600 dark:text-rose-400 rounded-2xl p-4 flex flex-col gap-y-1 shadow-sm">
        <div class="flex items-center gap-x-2.5 font-semibold">
            ...
            <span>{{ __('Veuillez corriger les erreurs suivantes :') }}</span>
        </div>
    ...
    <div class="flex flex-col border border-gray-200/60 dark:border-slate-800/60 rounded-2xl shadow-premium bg-white dark:bg-[#0D121F] overflow-hidden">
        ...
        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/30">
            ...
            <div class="sm:col-span-1 max-w-sm w-full relative">
                <input x-model="search" type="text" class="py-2.5 px-4 ps-11 block w-full border-gray-200 dark:border-slate-850 dark:bg-[#080B11] dark:text-slate-300 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="{{ __('Nom de l\'immeuble...') }}">
            ...
            <div class="sm:col-span-2 md:grow flex justify-end gap-x-3 relative" x-data="{ openVille: false }">
                <div class="relative inline-flex">
                    <button @click="openVille = !openVille" @click.outside="openVille = false" type="button" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-gray-200/80 bg-white/50 hover:bg-gray-50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-white dark:hover:bg-slate-900/50 shadow-sm transition-all duration-200">
                        <span x-text="filterVille === 'all' ? '{{ __('Filtrer par Ville') }}' : filterVille" class="truncate max-w-[150px]"></span>
                        <i data-lucide="chevron-down" :class="openVille ? 'rotate-180' : ''" class="size-4 transition-transform duration-200 text-gray-400 font-normal"></i>
                    </button>
                    <div x-show="openVille" x-cloak class="absolute right-0 top-full z-[100] mt-2 w-48 bg-white/95 dark:bg-[#0D121F]/95 border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md" style="display: none;">
                        <div @click="filterVille = 'all'; openVille = false" class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Toutes les villes') }}</div>
                        @foreach($villes as $v)
            ...
            <table class="min-w-full divide-y divide-gray-250 dark:divide-slate-800">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">{{ __('Adresse complète') }}</th>
                        <th scope="col" class="px-6 py-3.5 text-start text-xs font-bold text-slate-400 uppercase dark:text-neutral-450 tracking-wider">{{ __('Résidents &amp; Appartements') }}</th>
                ...
                <tbody class="divide-y divide-gray-200/60 dark:divide-slate-800/60">
                    ...
                    <tr x-show="matches('{{ addslashes($immeuble->nom) }}', '{{ addslashes($immeuble->ville) }}', 'Actif')" class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-700 dark:text-slate-300">
                            {{ $immeuble->appartements->count() }} {{ trans_choice('appartement|appartements', $immeuble->appartements->count()) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="w-full flex flex-col bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto">
                <div class="flex justify-between items-center py-4 px-6 border-b border-gray-200/60 dark:border-slate-800/60">
                    <div class="flex items-center gap-x-2.5">
                        ...
                        <h3 class="font-bold text-slate-800 dark:text-white" x-text="isEditing ? '{{ __('Modifier l\'Immeuble') }}' : '{{ __('Ajouter un Immeuble') }}'"></h3>
                    </div>
                ...
                <div class="p-6">
                    ...
                    <form :action="isEditing ? `/syndic/immeubles/${immeubleEnCours.id}` : '/syndic/immeubles'" method="POST">
                        ...
                        <div class="space-y-4">
                            <!-- Nom de l'immeuble -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 text-slate-700 dark:text-slate-200">{{ __('Nom de l\'immeuble') }}</label>
                                <input name="nom" x-model="immeubleEnCours.nom" type="text" class="py-2.5 px-4 block w-full border-gray-200 dark:border-slate-850 dark:bg-[#080B11] dark:text-slate-300 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="{{ __('Résidence Al Amal') }}" required>
                            </div>
                            
                            <!-- Assigner un Syndic responsable -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 text-slate-700 dark:text-slate-200">{{ __('Assigner un Syndic') }}</label>
                                ...
                                <div class="relative" x-data="{ openFormSyndic: false }">
                                    <button @click="openFormSyndic = !openFormSyndic" @click.outside="openFormSyndic = false" type="button" 
                                        ...
                                        <span x-text="immeubleEnCours.syndic_id ? syndicNames[immeubleEnCours.syndic_id] : '{{ __('Choisir un syndic responsable') }}'" class="truncate text-slate-700 dark:text-slate-300"></span>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-semibold mb-2 text-slate-700 dark:text-slate-200">{{ __('Ville') }}</label>
                                    ...
                                    <div x-show="!isAddingCustomVille" class="relative" x-data="{ openFormVille: false }">
                                        ...
                                        <button @click="openFormVille = !openFormVille" @click.outside="openFormVille = false" type="button"
                                            ...
                                            <span x-text="immeubleEnCours.ville ? immeubleEnCours.ville : '{{ __('Sélectionner une ville') }}'" class="truncate"></span>
                                    ...
                                    <div x-show="isAddingCustomVille" class="relative">
                                        <button @click="disableCustomVille()" type="button" title="{{ __('Retour à la liste') }}" class="absolute inset-y-0 start-0 flex items-center ps-3 text-slate-400 hover:text-primary-500 dark:hover:text-primary-400 transition-colors">
                                ...
                                <!-- Adresse complète -->
                                <div>
                                    <label class="block text-sm font-semibold mb-2 text-slate-700 dark:text-slate-200">{{ __('Adresse complète') }}</label>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold mb-2 text-slate-700 dark:text-slate-200">{{ __('Nombre d\'étages') }}</label>
                                ...
                                <div>
                                    <label class="block text-sm font-semibold mb-2 text-slate-700 dark:text-slate-200">{{ __('Nombre d\'appartements') }}</label>
                        ...
                        <div class="flex justify-end items-center gap-x-3 mt-6 pt-4 border-t border-gray-200/60 dark:border-slate-800/60">
                            <button type="button" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-slate-800 dark:bg-neutral-850 dark:border-neutral-800 dark:text-white dark:hover:bg-neutral-800 transition-all shadow-sm" data-hs-overlay="#hs-modal-add-immeuble">{{ __('Annuler') }}</button>
                            <button type="submit" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-bold rounded-xl border border-transparent bg-gradient-to-r from-primary-600 to-purple-600 text-white hover:from-primary-700 hover:to-purple-700 shadow-md shadow-primary-500/15 transition-all glow-hover" x-text="isEditing ? '{{ __('Enregistrer les modifications') }}' : '{{ __('Ajouter l\'immeuble') }}'"></button>
                        </div>
                    </form>
                </div>
            </div>
