...
<div x-data="syndicResidents({ items: [ @foreach($residents as $resident) { id: '{{ $resident->id }}', name: '{{ addslashes($resident->prenom) }} {{ addslashes($resident->nom) }}', immeuble: '{{ addslashes($resident->appartements->first() ? $resident->appartements->first()->immeuble->nom : 'N/A') }}' }, @endforeach ] })">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">{{ __('Gestion des Résidents') }}</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ __('Liste des copropriétaires et locataires associés à vos immeubles sous gestion.') }}</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200/60 dark:divide-slate-800/60">
                <thead class="bg-gray-50/50 dark:bg-[#090D16]/40">
                    <tr>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Nom complet') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Immeuble & Appt') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Statut du compte') }}</th>
                        <th class="px-6 py-4 text-end text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Action') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200/60 dark:divide-slate-800/60">
                    ...
                    <tr x-show="isRowVisible('{{ $resident->id }}', '{{ $resident->prenom }} {{ $resident->nom }}', '{{ $immeubleName }}')" class="hover:bg-gray-50/50 dark:hover:bg-slate-900/30 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @else
                                <span class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-full text-xs font-medium bg-amber-50/80 text-amber-600 dark:bg-amber-950/40 dark:text-amber-400 border border-amber-100 dark:border-amber-900/30">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <div class="inline-flex items-center gap-x-2">
                                <button @click="initEdit({{ json_encode($resident->id) }}, {{ json_encode($resident->prenom) }}, {{ json_encode($resident->nom) }}, {{ json_encode($resident->email) }}, {{ json_encode($resident->telephone) }}, {{ json_encode($resident->cin ?? '') }}, {{ json_encode($immeubleId) }}, {{ json_encode($appt ? $appt->numero : '') }}, {{ json_encode($appt ? $appt->pivot->date_entree : '') }}, {{ json_encode($resident->notes ?? '') }}, {{ json_encode($appt ? $appt->override_mois_retard : '') }}, {{ $appt ? $appt->cotisation_mensuelle : 0 }})" type="button" data-hs-overlay="#hs-modal-add-resident" class="py-1.5 px-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-lg border border-gray-200/80 hover:border-blue-300 dark:border-slate-800/80 dark:hover:border-blue-900/30 transition-all duration-200" title="{{ __('Modifier') }}">
                                    <i data-lucide="edit-2" class="size-4"></i>
                                </button>
                                <form action="{{ route('syndic.residents.destroy', $resident->id) }}" method="POST" onsubmit="return confirm('{{ __('Supprimer ce résident ?') }}');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
            <div class="w-full flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F]">
                <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
                    <h3 class="font-bold text-gray-800 dark:text-white text-lg" x-text="isEditing ? '{{ __('Modifier le Résident') }}' : '{{ __('Ajouter un Résident') }}'"></h3>
                    ...
                    <form :action="isEditing ? `/syndic/residents/${residentEnCours.id}` : '{{ route('syndic.residents.store') }}'" method="POST">
                        ...
                        <div class="grid gap-y-4">
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        <button @click="open = !open" @click.outside="open = false" type="button" class="py-2.5 px-4 flex justify-between items-center w-full border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 transition-all duration-200 text-left">
                                            <span x-text="residentEnCours.immeuble_id ? ({ @foreach($immeubles as $imm) '{{ $imm->id }}': '{{ addslashes($imm->nom) }}', @endforeach }[residentEnCours.immeuble_id] || '{{ __('Sélectionner un immeuble') }}') : '{{ __('Sélectionner un immeuble') }}'"></span>
                                ...
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Numéro d\'appartement') }}</label>
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Date d\'entrée') }}</label>
                                    <div class="relative">
                                        <input name="date_entree" x-model="residentEnCours.date_entree" x-init="flatpickr($el, { locale: '{{ app()->getLocale() }}', dateFormat: 'Y-m-d', onChange: (selectedDates, dateStr) => { residentEnCours.date_entree = dateStr; } }); $watch('residentEnCours.date_entree', val => $el._flatpickr.setDate(val))" type="text" class="py-2.5 px-4 ps-11 block w-full border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 transition-all duration-200" required>
                                        <div class="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-4">
                                ...
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Surcharge mois de retard') }}</label>
                                    <input name="override_mois_retard" x-model="residentEnCours.override_mois_retard" type="number" min="0" class="py-2.5 px-4 block w-full border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 transition-all duration-200" placeholder="{{ __('Automatique (laisser vide)') }}">
                                </div>
                            </div>
                            <div class="grid sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Cotisation mensuelle (DH)') }}</label>
                                    <input name="cotisation_mensuelle" x-model="residentEnCours.cotisation_mensuelle" type="number" step="0.01" min="0" class="py-2.5 px-4 block w-full border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 transition-all duration-200" placeholder="0.00" required>
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 pt-4">
                            <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-modal-add-resident">{{ __('Annuler') }}</button>
                            <button type="submit" class="py-2 px-4 text-sm font-semibold bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 text-white rounded-xl shadow-md shadow-primary-500/10 transition-all duration-300" x-text="isEditing ? '{{ __('Enregistrer les modifications') }}' : '{{ __('Ajouter le Résident') }}'"></button>
                        </div>
                    </form>
                </div>
            </div>
