...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">{{ __('Suivi des Paiements') }}</h2>
        <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ __('Vérifiez les versements des charges mensuelles des résidents.') }}</p>
    </div>
    <div class="flex flex-wrap gap-3">
        ...
        <a href="{{ route('syndic.paiements.export.excel') }}" class="py-2.5 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-emerald-600 text-white hover:bg-emerald-700 shadow-md shadow-emerald-500/10 hover:shadow-lg hover:shadow-emerald-500/20 transition-all duration-300 transform hover:-translate-y-0.5">
            <i data-lucide="file-spreadsheet" class="size-4"></i> {{ __('Export Excel') }}
        </a>
        <a href="{{ route('syndic.paiements.export.pdf') }}" target="_blank" class="py-2.5 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-rose-600 text-white hover:bg-rose-700 shadow-md shadow-rose-500/10 hover:shadow-lg hover:shadow-rose-500/20 transition-all duration-300 transform hover:-translate-y-0.5">
            <i data-lucide="file-text" class="size-4"></i> {{ __('Export PDF') }}
        </a>
    </div>
</div>
...
<div class="flex flex-col bg-white/80 dark:bg-[#0D121F]/90 border border-gray-200/60 dark:border-slate-800/60 rounded-2xl shadow-premium backdrop-blur-md">
    ...
    <div class="px-6 py-5 grid gap-4 md:flex md:justify-between md:items-center border-b border-gray-200/60 dark:border-slate-800/60 bg-white/40 dark:bg-[#0D121F]/40">
        ...
        <div class="sm:col-span-1 max-w-sm w-full relative">
            <label for="syndic-paiement-search" class="sr-only">{{ __('Rechercher') }}</label>
            <input x-model="search" type="text" id="syndic-paiement-search" class="py-2.5 px-4 ps-11 block w-full border-gray-200/80 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 dark:placeholder-neutral-500 transition-all duration-200" placeholder="{{ __('Rechercher un résident...') }}">
        ...
        <div class="sm:col-span-2 md:grow flex flex-wrap items-center justify-start sm:justify-end gap-2 relative">
            ...
            <div class="relative inline-flex">
              <button @click="openMois = !openMois" @click.outside="openMois = false" type="button" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-xl border border-gray-200/80 bg-white/50 hover:bg-gray-50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-white dark:hover:bg-slate-900/50 shadow-sm transition-all duration-200">
                <span x-text="moisSelectionne === 'all' ? '{{ __('Filtrer par Mois') }}' : moisSelectionne" class="truncate max-w-[120px]"></span>
              ...
              <div x-show="openMois" x-cloak class="absolute right-0 top-full z-[100] mt-2 w-48 bg-white/95 dark:bg-[#0D121F]/95 border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md">
                <div @click="moisSelectionne = 'all'; openMois = false" class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Tous les mois') }}</div>
            ...
            <div class="relative inline-flex">
              ...
              <div x-show="openImm" x-cloak class="absolute right-0 top-full z-[100] mt-2 w-56 bg-white/95 dark:bg-[#0D121F]/95 border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md">
                <div @click="immeubleSelectionne = 'all'; openImm = false" class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Tous les immeubles') }}</div>
            ...
            <div class="relative inline-flex">
              <button @click="openStat = !openStat" @click.outside="openStat = false" type="button" class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-xl border border-gray-200/80 bg-white/50 hover:bg-gray-50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-white dark:hover:bg-slate-900/50 shadow-sm transition-all duration-200">
                <span x-text="statutSelectionne === 'all' ? '{{ __('Statuts') }}' : statutSelectionne" class="truncate max-w-[120px]"></span>
              ...
              <div x-show="openStat" x-cloak class="absolute right-0 top-full z-[100] mt-2 w-48 bg-white/95 dark:bg-[#0D121F]/95 border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md">
                <div @click="statutSelectionne = 'all'; openStat = false" class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Tous les statuts') }}</div>
                <div @click="statutSelectionne = '{{ __('Payé') }}'; openStat = false" class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Payé') }}</div>
                <div @click="statutSelectionne = '{{ __('En retard') }}'; openStat = false" class="cursor-pointer w-full flex items-center py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-350 hover:bg-gray-150 dark:hover:bg-slate-800/50 transition-colors">{{ __('En retard') }}</div>
              </div>
            </div>
        </div>
    </div>
    ...
    <div class="overflow-x-auto rounded-b-2xl">
        <table class="min-w-full divide-y divide-gray-200/60 dark:divide-slate-800/60">
            <thead class="bg-gray-50/50 dark:bg-[#090D16]/40">
                <tr>
                    <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Résident') }}</th>
                    <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Mois') }}</th>
                    <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Montant') }}</th>
                    <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Statut') }}</th>
                    <th scope="col" class="px-6 py-4 text-end text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Action') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200/60 dark:divide-slate-800/60">
                ...
                <tr x-show="isRowVisible('{{ $item->id }}', '{{ addslashes($fullName) }}', '{{ $dateFr }}', '{{ addslashes($immeubleName) }}', '{{ $viewStatut }}')" class="hover:bg-gray-50/50 dark:hover:bg-slate-900/30 transition-colors duration-150 {{ $statutDb == 'impayé' ? 'bg-rose-50/[0.02]' : '' }}">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-x-3">
                            ...
                            <div class="grow">
                                ...
                                <span class="block text-xs text-gray-500 dark:text-slate-400">{{ $immeubleName }} - {{ __('Appt') }} {{ $apptNumero }}</span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="block text-sm font-semibold text-gray-800 dark:text-slate-200">{{ number_format($item->montant, 2) }} {{ __('MAD') }}</span>
                        @if($statutDb === 'partiel')
                            <span class="block text-xs text-amber-500 font-medium">{{ __('Payé') }}: {{ number_format($validatedPaymentsSum, 2) }} {{ __('MAD') }}</span>
                        @endif
                    </td>
    ...
    <div class="flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F] w-full overflow-hidden">
      ...
      <form action="{{ route('syndic.paiements.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-6 overflow-y-auto">
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Cotisation Sélectionnée') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Montant payé (MAD)') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Pièce jointe / Reçu (Optionnel - Image ou PDF)') }}</label>
        ...
        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 p-4">
          <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-saisir-paiement-modal">{{ __('Annuler') }}</button>
...
<div id="hs-gerer-paiements-modal" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none bg-slate-950/40 backdrop-blur-sm">
  <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-xl sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
    <div class="flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F] w-full overflow-hidden">
      ...
      <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
        <h3 class="font-bold text-gray-800 dark:text-white text-lg">
          {{ __('Versements enregistrés (Détails)') }}
        </h3>
      ...
      <div class="p-6 overflow-y-auto">
        ...
        <div class="mb-6 p-4 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-gray-150 dark:border-slate-800">
            <span class="block text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">{{ __('Cotisation concernée') }}</span>
            <span x-text="currentCharge ? currentCharge.titre : ''" class="block font-bold text-gray-800 dark:text-slate-200 mt-1"></span>
            <div class="mt-2 flex justify-between text-sm">
                <span class="text-gray-500 dark:text-slate-400">{{ __('Montant total requis :') }}</span>
                <span x-text="currentCharge ? parseFloat(currentCharge.montant).toFixed(2) + ' {{ __('MAD') }}' : ''" class="font-semibold text-gray-800 dark:text-white"></span>
            </div>
            <div class="mt-1 flex justify-between text-sm">
                <span class="text-gray-500 dark:text-slate-400">{{ __('Statut actuel :') }}</span>
                <span x-text="currentCharge ? currentCharge.statut === 'payé' ? '{{ __('Payé') }}' : (currentCharge.statut === 'partiel' ? '{{ __('Partiel') }}' : '{{ __('Impayé') }}') : ''" 
        ...
        <h4 class="text-sm font-semibold mb-3 dark:text-neutral-200">{{ __('Règlements saisis :') }}</h4>
        <div class="space-y-3">
                ...
                <div class="p-4 rounded-xl border border-gray-200/80 dark:border-slate-800/80 bg-white/50 dark:bg-[#0D121F]/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="space-y-1">
                        <div class="flex items-center gap-x-2">
                            <span x-text="parseFloat(p.montant).toFixed(2) + ' {{ __('MAD') }}'" class="font-bold text-slate-800 dark:text-white"></span>
                            <span class="text-xs px-2 py-0.5 rounded-full font-medium"
                                  :class="p.statut === 'validé' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'"
                                  x-text="p.statut === 'validé' ? '{{ __('Payé') }}' : '{{ __('En attente') }}'">
                            </span>
                        </div>
                        <div class="text-xs text-gray-500 dark:text-slate-400 flex flex-wrap gap-x-3 gap-y-1">
                            <span><strong class="font-medium text-gray-600 dark:text-slate-350">{{ __('Date :') }}</strong> <span x-text="formatDate(p.date_paiement)"></span></span>
                            <span><strong class="font-medium text-gray-600 dark:text-slate-350">{{ __('Mode :') }}</strong> <span x-text="p.mode_paiement"></span></span>
                        </div>
                    </div>
                    
                    {{-- Actions individuelles {{ __('sur') }} versement --}}
                    <div class="flex items-center gap-2">
                        <template x-if="p.recu_path">
                            <a :href="'/storage/' + p.recu_path" target="_blank" class="p-2 inline-flex items-center justify-center text-xs font-semibold rounded-lg border border-gray-200 dark:border-slate-800 text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-900" title="{{ __('Justificatif') }}">
                                <i data-lucide="paperclip" class="size-4"></i>
                            </a>
                        </template>

                        <template x-if="p.statut === 'validé'">
                            <a :href="'/syndic/paiements/' + p.id + '/receipt'" target="_blank" class="p-2 inline-flex items-center justify-center text-xs font-semibold rounded-lg border border-gray-200 dark:border-slate-800 text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-900" title="{{ __('Imprimer le Reçu') }}">
                        ...
                        <button type="button" 
                                ...
                                " 
                                class="p-2 inline-flex items-center justify-center text-xs font-semibold rounded-lg border border-blue-200/80 text-blue-600 hover:bg-blue-50/50 dark:border-blue-900/30 dark:text-blue-450 dark:hover:bg-blue-950/20" title="{{ __('Modifier ce versement') }}">
                            <i data-lucide="pencil" class="size-4"></i>
                        </button>

                        <button type="button" @click="deletePaiement(p.id)" class="p-2 inline-flex items-center justify-center text-xs font-semibold rounded-lg border border-rose-200/80 text-rose-600 hover:bg-rose-50/50 dark:border-rose-900/30 dark:text-rose-450 dark:hover:bg-rose-950/20" title="{{ __('Supprimer ce versement') }}">
            ...
            <template x-if="!currentCharge || currentCharge.paiements.length === 0">
                <div class="text-center py-6 text-sm text-gray-400 dark:text-slate-500">
                    {{ __('Aucun versement enregistré pour cette cotisation.') }}
                </div>
            </template>
        </div>
      </div>
...
<div id="hs-modifier-paiement-modal" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none bg-slate-950/40 backdrop-blur-sm">
  <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
    <div class="flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F] w-full overflow-hidden">
      <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
        <h3 class="font-bold text-gray-800 dark:text-white text-lg">
          {{ __('Modifier les détails du versement') }}
        </h3>
      ...
      <form :action="'/syndic/paiements/' + editPaiementId" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-6 overflow-y-auto">
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Cotisation / Résident Destinataire') }}</label>
                        ...
                        <div x-show="openEditSelectCharge" x-cloak class="absolute left-0 top-full z-[100] mt-2 w-full max-h-60 overflow-y-auto bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md">
                            @foreach($chargesList as $c)
                                @php
                                    ...
                                    $cLabel = $c->titre . " - " . $resName . " (" . ($c->appartement->immeuble->nom ?? '') . " - " . __('Appt') . " " . ($c->appartement->numero ?? '') . ")";
                                @endphp
                                <button type="button" 
                                        @click="
                                            editChargeId = '{{ $c->id }}';
                ...
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Montant versé (MAD)') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Date du versement') }}</label>
                    <div class="relative">
                        <input type="text" name="date_paiement" x-model="editDate" x-init="flatpickr($el, { locale: '{{ app()->getLocale() }}', dateFormat: 'Y-m-d', onChange: (selectedDates, dateStr) => { editDate = dateStr; } }); $watch('editDate', val => $el._flatpickr.setDate(val))" required class="py-2.5 px-4 ps-11 block w-full border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 transition-all duration-200">
                        <div class="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-4">
                            <i data-lucide="calendar" class="size-4 text-gray-400 dark:text-neutral-500"></i>
                        </div>
                    </div>
                ...
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Statut du Versement') }}</label>
                    <div class="relative w-full inline-flex">
                        <button type="button" @click="openEditStat = !openEditStat" @click.outside="openEditStat = false" class="py-2.5 px-4 w-full inline-flex justify-between items-center gap-x-2 text-sm font-semibold rounded-xl border border-gray-200/80 bg-white/50 hover:bg-white text-slate-800 shadow-sm dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-white dark:hover:bg-slate-900/50 transition-all duration-200">
                            <span x-text="editStatut === 'validé' ? '{{ __('Payé') }} (Validé)' : '{{ __('En attente') }}'" class="truncate text-left pr-4"></span>
                ...
                <div>
                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Pièce jointe / Preuve de virement (Optionnel - Image ou PDF)') }}</label>
                    <input type="file" name="piece_jointe" accept="image/*,application/pdf" class="py-2 px-3 block w-full border border-gray-200/80 dark:border-slate-800/80 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:text-neutral-300 file:bg-gray-100 file:border-0 file:me-4 file:py-1.5 file:px-3 file:rounded-lg file:text-xs file:font-semibold dark:file:bg-slate-800 dark:file:text-neutral-350 transition-all duration-200">
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end items-center gap-x-3 border-t border-gray-100 dark:border-slate-800/60 p-4 bg-gray-50/50 dark:bg-slate-900/10">
          <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-350 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl" data-hs-overlay="#hs-modifier-paiement-modal">{{ __('Annuler') }}</button>
          <button type="submit" class="py-2 px-4 text-sm font-semibold bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 text-white rounded-xl shadow-md shadow-primary-500/10 transition-all duration-300">
