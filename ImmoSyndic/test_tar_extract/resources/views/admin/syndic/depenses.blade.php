...
@section('content')
<div x-data="syndicDepenses" class="space-y-8">
    <!-- Page Header -->
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">{{ __('Dépenses de Copropriété') }}</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ __('Enregistrez et justifiez chaque centime dépensé pour vos copropriétés gérées.') }}</p>
        </div>
        <button @click="initAjout()" type="button" data-hs-overlay="#hs-modal-add-depense-syndic" class="py-2.5 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 text-white shadow-md shadow-primary-500/10 hover:shadow-lg hover:shadow-primary-500/20 transition-all duration-300 transform hover:-translate-y-0.5">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div class="p-6 bg-gradient-to-br from-primary-600 to-indigo-600 rounded-2xl shadow-premium text-white relative overflow-hidden group">
            ...
            <span class="block text-xs font-bold uppercase tracking-widest text-primary-200">{{ __('Budget Total Dépensé') }}</span>
            <span class="block text-3xl font-black mt-2 tracking-tight">{{ number_format($depenses->sum('montant'), 2) }} {{ __('DH') }}</span>
            <span class="block text-xs text-primary-100/80 mt-2 font-medium">{{ __('Sur vos :count opérations', ['count' => $depenses->count()]) }}</span>
        </div>
        ...
        <div class="p-6 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center gap-x-4">
            ...
            <div>
                <span class="block text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">{{ __('Mes Immeubles Gérés') }}</span>
                <span class="block text-2xl font-extrabold text-slate-850 dark:text-white mt-1">{{ $immeubles->count() }}</span>
            </div>
        </div>
        ...
        <div class="p-6 bg-white border border-gray-200/60 rounded-2xl shadow-premium dark:bg-[#0D121F] dark:border-slate-800/60 flex items-center gap-x-4">
            ...
            <div>
                <span class="block text-xs font-bold text-slate-400 dark:text-neutral-450 uppercase tracking-wider">{{ __('Justificatifs Fournis') }}</span>
                <span class="block text-2xl font-extrabold text-slate-850 dark:text-white mt-1">{{ $depenses->whereNotNull('justificatif_path')->count() }}</span>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200/60 dark:divide-slate-800/60">
                <thead class="bg-gray-50/50 dark:bg-[#090D16]/40">
                    <tr>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Désignation Dépense') }}</th>
                        ...
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Montant') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Date d\'opération') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Justificatif') }}</th>
                ...
                <tbody class="divide-y divide-gray-200/60 dark:divide-slate-800/60">
                    ...
                    <tr x-show="matches('{{ addslashes($depense->titre) }}', '{{ addslashes($immeubleNom) }}')" class="hover:bg-gray-50/50 dark:hover:bg-slate-900/30 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-rose-600 dark:text-rose-450">
                            - {{ number_format($depense->montant, 2) }} {{ __('DH') }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @else
                                <span class="text-xs text-gray-400 dark:text-slate-600 italic">{{ __('Non fourni') }}</span>
                            @endif
                        </td>

                        <!-- Actions -->
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <form action="{{ route('syndic.depenses.destroy', $depense->id) }}" method="POST" onsubmit="return confirm('Voulez-vous supprimer cette dépense ? Cette action supprimera définitivement le justificatif sur le disque.');" class="inline-block">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <span class="font-medium">{{ __('Aucune dépense enregistrée pour le moment.') }}</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <!-- Modal Form (Add Depense Syndic) -->
    <div id="hs-modal-add-depense-syndic" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none bg-slate-950/40 backdrop-blur-sm" role="dialog" tabindex="-1">
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
            <div class="w-full flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F]">
                ...
                <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
                    <h3 class="font-bold text-gray-800 dark:text-white text-lg">{{ __('Enregistrer une Dépense') }}</h3>
                    <button type="button" class="size-8 inline-flex justify-center items-center rounded-xl bg-gray-100 text-gray-800 hover:bg-gray-200 dark:bg-slate-800 dark:text-neutral-400 dark:hover:bg-slate-700 transition-colors" data-hs-overlay="#hs-modal-add-depense-syndic">
                ...
                <!-- Modal Body -->
                <div class="p-6">
                    <form action="{{ route('syndic.depenses.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        
                        <div class="grid gap-y-4">
                            <!-- Titre -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Désignation / Titre de la dépense') }}</label>
                                <input name="titre" x-model="depenseEnCours.titre" type="text" class="py-2.5 px-4 block w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/85 dark:text-neutral-300 transition-all duration-200" placeholder="Ex: Réparation Pompe d'eau..." required>
                            </div>

                            <!-- Immeuble Selector (Custom Alpine Dropdown) -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Immeuble concerné') }}</label>
                                <div class="relative">
                                    <button @click="openFormImm = !openFormImm" @click.outside="openFormImm = false" type="button" class="py-2.5 px-4 flex justify-between items-center w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/85 dark:text-neutral-300 transition-all duration-200 text-left">
                                        <span x-text="depenseEnCours.immeuble_id ? ({ @foreach($immeubles as $imm) '{{ $imm->id }}': '{{ addslashes($imm->nom) }}', @endforeach }[depenseEnCours.immeuble_id] || '{{ __('Sélectionner un immeuble') }}') : '{{ __('Sélectionner un immeuble') }}'"></span>
                                    ...
                                    <div x-show="openFormImm" x-cloak class="absolute left-0 top-full z-[100] mt-2 w-full max-h-60 overflow-y-auto bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md" style="display: none;">
                                        @foreach($immeubles as $immeuble)
                                            <div @click="depenseEnCours.immeuble_id = '{{ $immeuble->id }}'; openFormImm = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ $immeuble->nom }}</div>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <!-- Montant -->
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Montant (DH)') }}</label>
                                    <input name="montant" x-model="depenseEnCours.montant" type="number" step="0.01" class="py-2.5 px-4 block w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/85 dark:text-neutral-300 transition-all duration-200" placeholder="Ex: 1200.00" required>
                                </div>

                                <!-- Date -->
                                <div>
                                    <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Date d\'opération') }}</label>
                                    <div class="relative">
                                        <input name="date_depense" x-model="depenseEnCours.date_depense" x-init="flatpickr($el, { locale: '{{ app()->getLocale() }}', dateFormat: 'Y-m-d', onChange: (selectedDates, dateStr) => { depenseEnCours.date_depense = dateStr; } }); $watch('depenseEnCours.date_depense', val => $el._flatpickr.setDate(val))" type="text" class="py-2.5 px-4 ps-11 block w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/85 dark:text-neutral-300 transition-all duration-200" required>
                                        <div class="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-4">
                            ...
                            <!-- Description -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Description / Détails') }}</label>
                                <textarea name="description" x-model="depenseEnCours.description" rows="3" class="py-2.5 px-4 block w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/85 dark:text-neutral-300 transition-all duration-200" placeholder="{{ __('Entrez des détails ou raisons sur cette dépense...') }}"></textarea>
                            </div>

                            <!-- Justificatif -->
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Télécharger le justificatif (Image/PDF)') }}</label>
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-slate-800 border-dashed rounded-xl hover:bg-gray-50/50 dark:hover:bg-slate-900/10 transition-colors cursor-pointer relative group">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600 dark:text-slate-400 justify-center">
                                            <label for="justificatif-upload-syndic" class="relative cursor-pointer rounded-md font-semibold text-primary-600 hover:text-primary-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary-500">
                                                <span>{{ __('Télécharger le justificatif') }}</span>
                                                <input id="justificatif-upload-syndic" name="justificatif" type="file" class="sr-only" onchange="document.getElementById('fileNameSpanSyndicDep').innerText = this.files[0] ? this.files[0].name : '{{ __('Aucun fichier sélectionné') }}'">
                                            </label>
                                        </div>
                                        <p class="text-xs text-gray-500 dark:text-slate-500">{{ __('PDF, PNG, JPG jusqu\'à 20 Mo') }}</p>
                                        <div id="fileNameDisplaySyndicDep" class="mt-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 py-1 px-2 rounded-lg inline-block">
                                            <span id="fileNameSpanSyndicDep">{{ __('Aucun fichier sélectionné') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 pt-4">
                            <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-modal-add-depense-syndic">{{ __('Annuler') }}</button>
                            <button type="submit" class="py-2 px-4 text-sm font-semibold bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 text-white rounded-xl shadow-md shadow-primary-500/10 transition-all duration-300">{{ __('Enregistrer') }}</button>
                        </div>
                    </form>
                </div>
            </div>
