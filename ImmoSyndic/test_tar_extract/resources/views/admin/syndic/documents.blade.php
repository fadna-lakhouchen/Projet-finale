...
<div x-data="syndicDocuments" class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">{{ __('Documents de Copropriété') }}</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ __('Gérez le coffre-fort documentaire et partagez des pièces officielles avec vos résidents.') }}</p>
        </div>
                    ...
                    <tr x-show="matches('{{ addslashes($doc->titre) }}', '{{ addslashes($immeubleNom) }}', '{{ $doc->categorie }}')" class="hover:bg-gray-50/50 dark:hover:bg-slate-900/30 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <div class="inline-flex items-center gap-x-2">
                                ...
                                <!-- Delete -->
                                <form action="{{ route('syndic.documents.destroy', $doc->id) }}" method="POST" onsubmit="return confirm('{{ __('Voulez-vous vraiment supprimer définitivement ce document ?') }}');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <span class="font-medium">{{ __('Aucun document stocké pour le moment.') }}</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
                ...
                <div class="p-6">
                    <form action="{{ route('syndic.documents.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <div x-show="openFormCat" x-cloak class="absolute left-0 top-full z-[100] mt-2 w-full bg-white dark:bg-[#0D121F] border border-gray-200/60 dark:border-slate-800/60 shadow-xl rounded-xl p-1.5 backdrop-blur-md" style="display: none;">
                                        <div @click="documentEnCours.categorie = 'Facture'; openFormCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Facture') }}</div>
                                        <div @click="documentEnCours.categorie = 'Contrat'; openFormCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Contrat & Devis') }}</div>
                                        <div @click="documentEnCours.categorie = 'PV'; openFormCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Rapport de Réunion (PV)') }}</div>
                                        <div @click="documentEnCours.categorie = 'Autre'; openFormCat = false" class="cursor-pointer py-2 px-3 rounded-lg text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors">{{ __('Autre') }}</div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600 dark:text-slate-400 justify-center">
                                            <label for="file-upload-syndic" class="relative cursor-pointer rounded-md font-semibold text-primary-600 hover:text-primary-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary-500">
                                                ...
                                                <input id="file-upload-syndic" name="fichier" type="file" class="sr-only" required onchange="document.getElementById('fileNameSpanSyndic').innerText = this.files[0] ? this.files[0].name : '{{ __('Aucun fichier sélectionné') }}'">
                                            </label>
                                        </div>
                                        ...
                                        <div id="fileNameDisplaySyndic" class="mt-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 py-1 px-2 rounded-lg inline-block">
                                            <span id="fileNameSpanSyndic">{{ __('Aucun fichier sélectionné') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 pt-4">
                            <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-modal-upload-document-syndic">{{ __('Annuler') }}</button>
