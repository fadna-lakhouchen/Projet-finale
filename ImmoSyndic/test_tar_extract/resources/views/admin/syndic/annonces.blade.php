...
<div x-data="syndicAnnonces">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight dark:text-white">{{ __('Gestion des Annonces') }}</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ __('Publiez des annonces et avertissements pour informer instantanément les résidents de vos immeubles.') }}</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200/60 dark:divide-slate-800/60">
                <thead class="bg-gray-50/50 dark:bg-[#090D16]/40">
                    <tr>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Annonce') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Immeuble ciblé') }}</th>
                        <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-slate-400">{{ __('Date de publication') }}</th>
                    ...
                    <tr x-show="matches('{{ addslashes($annonce->titre) }}', '{{ addslashes($immeubleNom) }}')" class="hover:bg-gray-50/50 dark:hover:bg-slate-900/30 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <div class="inline-flex items-center gap-x-2">
                                <button @click="initEdit({{ json_encode($annonce->id) }}, {{ json_encode($annonce->titre) }}, {{ json_encode($annonce->contenu) }}, {{ json_encode($immeubleId) }})" type="button" data-hs-overlay="#hs-modal-add-annonce" class="py-1.5 px-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-lg border border-gray-200/80 hover:border-blue-300 dark:border-slate-800/80 dark:hover:border-blue-900/30 transition-all duration-200" title="{{ __('Modifier') }}">
                                    <i data-lucide="edit-2" class="size-4"></i>
                                </button>
                                <form action="{{ route('syndic.annonces.destroy', $annonce->id) }}" method="POST" onsubmit="return confirm('{{ __('Supprimer définitivement cette annonce ?') }}');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-sm text-gray-500 dark:text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <span>{{ __('Aucune annonce publiée pour le moment.') }}</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
            <div class="w-full flex flex-col bg-white border border-gray-200/60 dark:border-slate-800/60 shadow-premium rounded-2xl pointer-events-auto dark:bg-[#0D121F]">
                <div class="flex justify-between items-center py-4 px-5 border-b border-gray-200/60 dark:border-slate-800/60">
                    <h3 class="font-bold text-gray-800 dark:text-white text-lg" x-text="isEditing ? '{{ __("Modifier l'Annonce") }}' : '{{ __('Publier une Annonce') }}'"></h3>
                ...
                <div class="p-6 overflow-y-auto">
                    <form :action="isEditing ? `/syndic/annonces/${annonceEnCours.id}` : '{{ route('syndic.annonces.store') }}'" method="POST">
                        ...
                        <div class="grid gap-y-4">
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Titre de l\'annonce') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Immeuble ciblé') }}</label>
                                <div class="relative" x-data="{ open: false }">
                                    <button @click="open = !open" @click.outside="open = false" type="button" class="py-2.5 px-4 flex justify-between items-center w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 transition-all duration-200 text-left">
                                        <span x-text="annonceEnCours.immeuble_id ? ({ @foreach($immeubles as $imm) '{{ $imm->id }}': '{{ addslashes($imm->nom) }}', @endforeach }[annonceEnCours.immeuble_id] || '{{ __('Sélectionner un immeuble') }}') : '{{ __('Sélectionner un immeuble') }}'"></span>
                            ...
                            <div>
                                <label class="block text-sm font-semibold mb-2 dark:text-neutral-200">{{ __('Description / Contenu') }}</label>
                                <textarea name="contenu" x-model="annonceEnCours.contenu" rows="5" class="py-2.5 px-4 block w-full border border-gray-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 bg-white/50 dark:bg-[#090D16]/50 dark:border-slate-800/80 dark:text-neutral-300 transition-all duration-200" placeholder="{{ __('Décrivez le contenu détaillé de l\'annonce...') }}" required></textarea>
                            </div>
                        </div>

                        <div class="flex justify-end items-center gap-x-3 mt-6 border-t border-gray-100 dark:border-slate-800/60 pt-4">
                            <button type="button" class="py-2 px-4 text-sm font-medium border border-gray-200 dark:border-slate-800 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-slate-900 rounded-xl transition-colors duration-150" data-hs-overlay="#hs-modal-add-annonce">{{ __('Annuler') }}</button>
                            <button type="submit" class="py-2 px-4 text-sm font-semibold bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 text-white rounded-xl shadow-md shadow-primary-500/10 transition-all duration-300" x-text="isEditing ? '{{ __('Enregistrer les modifications') }}' : '{{ __('Publier') }}'"></button>
                        </div>
                    </form>
                </div>
            </div>
