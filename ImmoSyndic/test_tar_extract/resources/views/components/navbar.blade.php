    ...
    <nav class="flex basis-full items-center w-full mx-auto px-4 sm:px-6" aria-label="Global">
        ...
        <div class="flex items-center gap-x-3 lg:hidden me-5">
            ...
            <a class="flex items-center gap-x-2 text-lg font-bold dark:text-white shrink-0" href="{{ route('dashboard') }}">
                ...
                <span class="bg-gradient-to-r from-primary-500 to-purple-600 bg-clip-text text-transparent">ImmoSyn</span>
            </a>
        </div>
        ...
        <div class="w-full flex items-center justify-end ms-auto lg:justify-between lg:gap-x-3">
            ...
            <div class="hidden lg:flex items-center gap-x-3">
                <h1 class="text-base font-semibold text-slate-800 dark:text-white flex items-center gap-1.5">
                    {{ __('Bonjour') }}, <span class="bg-gradient-to-r from-primary-600 to-purple-600 bg-clip-text text-transparent font-bold">{{ auth()->user()->prenom }} {{ auth()->user()->nom }}</span>
                </h1>
                @php
                    $roleColor = match(auth()->user()->active_role) {
                        'administrateur', 'admin' => 'bg-red-500/10 text-red-500 border border-red-500/20',
            ...
            <div class="flex flex-row items-center justify-end gap-2.5">
                ...
                <div x-data="{ openNotif: false }" class="relative inline-flex">
                    ...
                    <div x-show="openNotif" 
                         ...
                         class="absolute {{ app()->getLocale() === 'ar' ? 'left-0' : 'right-0' }} top-full z-50 min-w-80 max-w-sm bg-white shadow-xl border border-gray-150 rounded-2xl p-2 mt-2 dark:bg-neutral-900 dark:border-neutral-800" 
                         style="display: none;">
                        
                        <!-- Header -->
                        <div class="flex items-center justify-between py-2 px-4 border-b border-gray-100 dark:border-neutral-800 pb-2 mb-2">
                            <h4 class="font-bold text-slate-800 dark:text-white">{{ __('Notifications') }}</h4>
                            @if($unreadCount > 0)
                                <form action="{{ route('notifications.read') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="text-xs font-bold text-primary-600 hover:text-primary-500 dark:text-primary-400">{{ __('Tout marquer comme lu') }}</button>
                                </form>
                            @endif
                        </div>
                        ...
                        <div class="max-h-72 overflow-y-auto space-y-1 scrollbar-none">
                            @forelse($notifications as $notif)
                                <div class="p-3 rounded-xl transition-colors hover:bg-slate-50 dark:hover:bg-neutral-800/50 flex gap-x-3 {{ !$notif->lu ? 'bg-primary-500/[0.03] dark:bg-primary-500/[0.05]' : '' }}">
                                    ...
                                    <div class="flex flex-col min-w-0 flex-1">
                                        <p class="text-xs font-bold text-slate-800 dark:text-white truncate">{{ $notif->translated_titre }}</p>
                                        <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 leading-normal line-clamp-2">{{ $notif->translated_message }}</p>
                            ...
                            @empty
                                <div class="py-6 text-center text-xs text-slate-450 dark:text-neutral-500">
                                    ...
                                    <span>{{ __('Aucune notification récente.') }}</span>
                                </div>
                            @endforelse
                        </div>
                    </div>
                ...
                <div x-data="{ open: false }" class="relative inline-flex">
                    ...
                    <div x-show="open" 
                         ...
                         x-transition:leave-start="transform opacity-100 scale-100"
                         x-transition:leave-end="transform opacity-0 scale-95"
                         class="absolute {{ app()->getLocale() === 'ar' ? 'left-0' : 'right-0' }} top-full z-50 min-w-60 bg-white shadow-xl border border-gray-150 rounded-2xl p-2 mt-2 dark:bg-neutral-900 dark:border-neutral-800" 
                         style="display: none;">
                        <div class="py-3 px-5 -m-2 bg-gradient-to-r from-primary-50/50 to-purple-50/30 rounded-t-2xl mb-2 dark:from-neutral-800 dark:to-neutral-800/80 border-b border-gray-100 dark:border-neutral-800">
                            <p class="text-xs text-slate-400 dark:text-neutral-450 uppercase font-bold tracking-wider mb-0.5">{{ __('Identifié en tant que') }}</p>
                            <p class="text-sm font-semibold text-slate-800 dark:text-white truncate">{{ auth()->user()->email }}</p>
                        </div>
                        @php
                            $paramsRoute = match(auth()->user()->active_role) {
                                'administrateur', 'admin' => route('admin.parametres'),
                        ...
                        <a class="flex items-center gap-x-3 py-2 px-3 rounded-xl text-sm text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800 transition-colors" href="{{ $paramsRoute }}">
                            <i data-lucide="settings" class="size-4 text-slate-400"></i> {{ __('Paramètres du profil') }}
                        </a>
                        @endif
                        @if(auth()->user()->hasMultipleRoles())
                            <hr class="my-2 border-gray-100 dark:border-neutral-800">
                            <div class="px-3 py-1.5 text-[10px] font-bold text-slate-400 dark:text-neutral-500 uppercase tracking-wider">
                                {{ __('Changer d\'espace') }}
                            </div>
                            @if(auth()->user()->active_role !== 'syndic' && (auth()->user()->hasRole('syndic') || auth()->user()->role === 'syndic'))
                                <a class="flex items-center gap-x-3 py-2 px-3 rounded-xl text-sm text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800 transition-colors" href="{{ route('role.switch', 'syndic') }}">
                                    <i data-lucide="shield" class="size-4 text-primary-500"></i> {{ __('Basculer vers l\'Espace Syndic') }}
                                </a>
                            @endif
                            @if(auth()->user()->active_role !== 'resident' && (auth()->user()->hasRole('resident') || auth()->user()->role === 'resident'))
                                <a class="flex items-center gap-x-3 py-2 px-3 rounded-xl text-sm text-slate-700 hover:bg-slate-50 dark:text-neutral-350 dark:hover:bg-neutral-800 transition-colors" href="{{ route('role.switch', 'resident') }}">
                                    <i data-lucide="home" class="size-4 text-green-500"></i> {{ __('Basculer vers l\'Espace Résident') }}
                                </a>
                            @endif
                        @endif
                        <hr class="my-2 border-gray-100 dark:border-neutral-800">
                        <form action="{{ route('logout') }}" method="POST">
                            ...
                            <button type="submit" class="w-full flex items-center gap-x-3 py-2 px-3 rounded-xl text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors">
                                <i data-lucide="log-out" class="size-4"></i> {{ __('Déconnexion') }}
                            </button>
                        </form>
                    </div>
                </div>
